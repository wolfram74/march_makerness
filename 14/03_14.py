...
def make_output(int):
	...
	file = os.path.join(top_dir,date)+"_%s.scad" % int
	return open(file,"w")
...
def path_step(list, m):
	terminal = list[-1]
	a = (terminal[0]+terminal[1])%m
	b = (terminal[1] + a)%m
	new_end = (a,b)
	#print new_end
	if (a,b) in list:
...
def path_finder(start, mod):
	run = True
	path = [start]
	while run:
		run, path = path_step(path, mod)
	return path
...
### graphics
def colorout2(m, n): # using trig functions, m<n
    p=float(m)/float(n)#p is the normalized value of m.
    rp=math.cos(p*math.pi/2)**2
    gp=math.sin(p*math.pi)
    bp=math.sin(p*math.pi/2)**2
    r=int(255*rp)
    g=int(255*gp)
    b=int(255*bp)
    color=(r,g,b)
