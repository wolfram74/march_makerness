...
base = solid.color([0,0,0])(solid.cube([60,120,5]))
base = solid.translate([0,-60,0])(base)
system ={"sun": [1.41*10**18, 0, t55_to_1([255,255,0]) ,0],

"mercury": [6.08*10**10 , 57.9*10**6, t55_to_1([238,221,130]), 1],

"venus": [9.38*10**11 , 108*10**6, t55_to_1([189,183,107]), 2],

"earth": [1.08*10**12 , 149*10**6, t55_to_1([0,191,255]) , 3],

"moon": [2.19*10**10 , .384*10**6, t55_to_1([ 205,205,193]) ,4],

"mars": [1.63*10**11 , 228*10**6, t55_to_1([178,34,34]),4],

"jupiter": [1.43*10**15 ,778*10**6, t55_to_1([245,222,179]) ,5],

"saturn": [8.27*10**14 ,1.43*10**9, t55_to_1([255,165,0]) ,6],

"uranus": [6.83*10**13 ,2.87*10**9, t55_to_1([0,255,255]) ,7],

"neptune": [6.25*10**13 , 4.50*10**9, t55_to_1([0,0,205]) ,8]
}
object = []

for i in system.keys():
# 	try:
# 		print i, math.log(system[i][0])/10, 50*system[i][1]**(1.0/3)/1650, '\n'
# 	except:
# 		print i, system[i][0]**(1.0/3), 0, '\n'
	if i != "moon":
		ori = 50*system[i][1]**(1.0/3)/1650
		pri = math.log(system[i][0])/10
		orbit = solid.translate([0,0,-.5])(solid.cylinder(r= ori+.5, h=7.5))-solid.cylinder(r= ori-.5, h=8)
		planet = solid.sphere(r = pri, segments = 60)
		planet = solid.rotate([0,0,20*system[i][-1]-90])(solid.translate([ori,0,5])(planet))
		planet = solid.color(system[i][2])(planet)
		object += [orbit, planet]
object +=[base]

object= join_list(object)-solid.translate([-60,-60,-3])(solid.cube([60,120,14]))

print>>out_file, solid.scad_render(object)
